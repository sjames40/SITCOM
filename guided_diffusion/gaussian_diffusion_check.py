...
class GaussianDiffusion:
    ...
    def optimize_input(self, x, model, time, measurement,num_steps=5, learning_rate=0.01):
        ...
        for step in range(num_steps):
            optimizer.zero_grad()
            noise_pred = model(input_tensor, self._scale_timesteps(time))
            print(noise_pred.shape)
            pred_x0 = (input_tensor.to(device) -self.sqrt_one_minus_alphas_cumprod * noise_pred) / self.alphas_cumprod
            ...
            loss = torch.norm(operator.forward(sample, mask=mask)-measurement) / (num_steps*50)
            loss.backward()
            optimizer.step()
        noise = torch.randn_like(x)
        #if time != 0:  # no noise when t == 0
        #    pred_x0 += torch.exp(0.5 * out['log_variance']) * noise
        return {'sample': pred_x0, 'pred_xstart':  pred_x0 }
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            img =  out * self.alphas_cumprod_prev**0.5+ (1-self.alphas_cumprod_prev)**0.5*torch.randn(img.size()).to(device)
            img = img.detach_()
