...
class GaussianDiffusion:
    ...
    def optimize_input(self, x, model, time, measurement,num_steps=5, learning_rate=0.01):
        input_tensor = torch.randn(1, 3, 256, 256, requires_grad=True)
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        task_config= '/home/shijun.liang/github_code/diffusion-posterior-sampling-main/configs/inpainting_config.yaml'
        task_config = load_yaml(task_config)
        measure_config = task_config['measurement']
        operator = get_operator(device=device, **measure_config['operator'])
        noiser = get_noise(**measure_config['noise'])
        input_tensor.data = x.clone().to(device)
        #input_tensor = x
        ...
        for step in range(num_steps):
            optimizer.zero_grad()
            #pred =  self.p_sample(x=input_tensor, t=time, model=model)
            #pred_x0 = pred['sample']
            out = self.p_mean_variance(model=model, x=input_tensor, t=time)
            sample = out['mean']
            #loss = torch.norm(noiser(operator.forward(sample, mask=mask))-measurement)/(num_steps*200)
            loss = torch.norm(operator.forward(sample, mask=mask)-measurement) / (num_steps*100)
            #loss = torch.norm(operator.forward(sample, mask=mask)-measurement) / (num_steps*300)
        ...
        #input_tensor = input_tensor.detach_()
        return {'sample': sample.detach(), 'pred_xstart': out['pred_xstart']}
    ...
    def p_sample_loop(self,
        ...
        pbar = tqdm(range(self.num_timesteps-1,-1,-2))
        for idx in pbar:
            time = torch.tensor([idx] * img.shape[0], device=device)
            img = img.requires_grad_()
            #out = self.optimize_input(img.clone(), model, time,measurement, num_steps=2, learning_rate=0.01)
            out = self.optimize_input(img, model, time,measurement, num_steps=2, learning_rate=0.01)
            # Give condition.
            ...
            if record:
                if idx % 5 == 0:
                    file_path = os.path.join(save_root, f"progress2/x_{str(idx).zfill(4)}.png")
    ...
    def p_mean_variance(self, model, x, t):
        ...
        #     file_path = os.path.join(out_path , f"progress/x_{str(t.cpu().detach().numpy()[0]).zfill(4)}.png")
        #     plt.imsave(file_path, clear_color(model_output[0]))
