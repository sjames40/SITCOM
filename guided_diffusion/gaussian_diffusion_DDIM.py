...
class GaussianDiffusion:
    ...
    def optimize_input(self, x, model, time, measurement,num_steps=5, learning_rate=0.01):
        ...
        for step in range(num_steps):
            ...
            eps = self.predict_eps_from_x_start(input_tensor, time, out['pred_xstart'])
            
            alpha_bar = extract_and_expand(self.alphas_cumprod, time, input_tensor)
            alpha_bar_prev = extract_and_expand(self.alphas_cumprod_prev, time, input_tensor)
            eta= 0.5
            sigma = (
                eta
            ...
            sample = mean_pred
            loss = torch.norm(operator.forward(sample, mask=mask)-measurement) #/ (num_steps*10) 
            loss.backward()
            optimizer.step()
        noise = torch.randn_like(x)
